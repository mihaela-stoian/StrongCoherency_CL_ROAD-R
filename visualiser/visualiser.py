...
from qualitative import find_violations, draw_rectangle
...
#
# FULL_IDS = ['2015-02-03-08-45-10_stereo_centre_0401039', '2014-12-10-18-10-50_stereo_centre_0201628']
# REQ_LABELS = ['MovAway', 'MovTow', 'Cyc']
...
def load_preds(args, preds_path:str, counter_per_img=3):
    ...
    with open(preds_path, 'r') as f:
        for line in f:
            ...
            if args.constrained:
                if not len(set(pos_labels).intersection(set(REQ_LABELS))) >= 1: #>= 1: #len(REQ_LABELS):
                    continue
            else:
            ...
            if pos_labels:
                if not args.constrained:
                    if found_viol:
                        fig = draw_rectangle(f"/users-2/mihaela/datasets/road/rgb-images/{videoname}/{img_id}.jpg", bbox_coords, preds, th=args.th, found_violations=found_viol, constrained=args.constrained)
                    else:
                        continue
                else:
                    fig = draw_rectangle(f"/users-2/mihaela/datasets/road/rgb-images/{videoname}/{img_id}.jpg", bbox_coords, preds, th=args.th, found_violations=found_viol, constrained=args.constrained)
            ...
            fig.savefig(f"{savepath}/{videoname}_{img_id}_{1 if 'TL' in pos_labels else 0}_{counter}_{args.constrained}.png", format='png', dpi=100)
